...
function autocomplete(input, dictionary) {
  let result = [];
  input = input.toLowerCase().replace(/[\W_\d]/g, '');
  result = dictionary.filter(item => item.toLowerCase().indexOf(input) === 0);
  return result.slice(0, 5);
}

describe("Solution", function(){
  it("should test for something", function(){
    assert.deepEqual(autocomplete('ai', ['airplane','airport','apple','ball']), ['airplane','airport'], "String ai");
  });
});
