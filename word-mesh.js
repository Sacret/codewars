...
const findIntersection = (word1, word2) => {
  const firstPosition = word1.indexOf(word2.charAt(0));
  if (firstPosition === -1) return false;
  const firstPart = word1.slice(firstPosition);

  return word2.startsWith(firstPart)
    ? firstPart
    : findIntersection(firstPart.slice(1), word2);
}
...
const wordMesh = arr => {
  ...
  for (let i = 0; i < arr.length - 1; i++) {
    const curr = arr[i];
    const next = arr[i + 1];
    const intersection = findIntersection(curr, next);
    if (intersection) {
      meshes += intersection;
    } else {
