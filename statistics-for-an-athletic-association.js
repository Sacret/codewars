/**
  * You are the "computer man" of a local Athletic Association (C.A.A.). Many teams of runners come to compete.
  ...
  *
  * Each part of the string is of the form: h|m|s where h, m, s are positive or null integer (represented as strings) with one or two digits.
...
function stat(strg) {
  ...
  const resultStr = strg.split(', ');

  const result = resultStr
    .map(res => moment(res, 'HH|mm|ss'))
    .sort((momA, momB) => momA - momB);

  const range = moment(result[result.length - 1] - result[0]).format('HH|mm|ss');
  const diffs = result.reduce((time, currMom) => {
    return time + currMom.diff(moment().startOf('day'));
  }, 0);
  const average = moment(diffs / result.length).format('HH|mm|ss');
  const resultHalfIndex = result.length / 2;
  const resMedian = result.length % 2 !== 0 ?
    result[(resultHalfIndex ^ 0)] :
    moment((result[resultHalfIndex - 1] + result[resultHalfIndex]) / 2);
  const median = resMedian.format('HH|mm|ss');
